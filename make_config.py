...
admin_user_id = os.popen("keystone user-list | grep admin | head -n 1 | cut -d'|' -f 2").read().strip()
...
neutron_public_url = os.popen("keystone endpoint-list | grep 9696 | head -n 1 | cut -d'|' -f 4").read().strip()
